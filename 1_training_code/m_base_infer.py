    ...
    with torch.no_grad():
        for __, (raw_line, batch_texta_source, batch_textb_source) in enumerate(tqdm(dataloader), start=1):
            ...
            if args.infer_task == common_def.INFER_TASK_TYPE_EMB:
                ...
                texta_emb = model(
                    input_ids=input_ids_texta, attention_mask=attention_mask_texta, token_type_ids=token_type_ids_texta,
                )
                ...
                textb_emb = model(
                    input_ids=input_ids_textb, attention_mask=attention_mask_textb, token_type_ids=token_type_ids_textb,
                )
...
def main():
    ...
    model = SentenceSimModel(args.pretrain_model_path, emb_dim=args.emb_dim, pooling_method=args.pooling_method).to(
        device
    )
